...
class Zoom_Advanced(ttk.Frame):
	...
	def configurate_canvas(self,nom,resolution,images,xmove=0.0,ymove=0.0):
		...
		self.create_images_array(nom,resolution,images)
		...
		self.width = self.images_array[0][0].size[0] * 2
		self.height = self.images_array[0][0].size[1] * 2
	...
	def create_images_array(self,nom,resolution,images):
		self.dimX = 0
		self.dimY = 0
		count= 0
		split = []
		...
		for image in images:
			...
			split = split.split('_')
			if split[0]==nom and split[1]==resolution:
				liste[int(split[2])][int(split[3])] = Image.open(repertoire + image)

		self.images_array = liste
		self.canvas.images_id = np.zeros((self.dimX + 1,self.dimY + 1),dtype = object)
	...
	def show_image(self, event=None):
		...
		if tx1 == tx2 :
			...
			if ty1 == ty2 :
				image00 = self.images_array[tx1][ty1].crop((int((x1 - xt)/ self.imscale), int((y1 - yt)/ self.imscale), 
				...
				self.canvas.images_id[tx1][ty1] = imagetk00  # keep an extra reference to prevent garbage-collection
				...
				imageid00 = self.canvas.create_image(max(bbox2[0], bbox1[0]), max(bbox2[1], bbox1[1]),
					anchor='nw', image=imagetk00, tags ='r')
				self.canvas.lower(imageid00)  # set image into background
				self.canvas.images_id[tx1][ty1] = imagetk00

				if qy2 > 0:

					image01 = self.images_array[tx1][ty2].crop((int((x1 -xt)/ self.imscale),0,
					...
					self.canvas.images_id[tx1][ty2] = imagetk01
			...
			if ty1 == ty2:
				...
				if qx2 > 0:
					...
					image10 = self.images_array[tx2][ty1].crop((0,int((y1 -yt)/ self.imscale),
					...
					self.canvas.images_id[tx2][ty1] = imagetk10
				...
				if qx2 > 0 and qy2 > 0:
					...
					image11 = self.images_array[tx2][ty2].crop((0,0,
					...
					self.canvas.images_id[tx2][ty2] = imagetk11
